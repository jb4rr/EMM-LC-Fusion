...
class MRIdataset(Dataset):
    def __init__(self, csv_file, descriptor, root_dir, image_size, transforms=None):
        super(MRIdataset, self).__init__()
        self.root_dir = root_dir + r'\SCANS'
        self.image_size = np.asarray(image_size)

        self.task = -2  # -2=cancer; -1=nodule/mass

        labels = pd.read_csv(os.path.join(root_dir, csv_file))
        self.labels = labels.set_index('patient_id').T.to_dict('list')
        descriptor = pd.read_csv(os.path.join(root_dir, descriptor))
